...
// Grouped by how often each is used: the daily clinic-running screens first,
// then the settings that change now and then, then the public-website content
// that is only touched occasionally.
const NAV_GROUPS: { label?: string; items: NavItem[] }[] = [
  { items: [{ id: 'overview', label: 'Dashboard', icon: <LayoutGrid className="w-4 h-4" /> }] },
  {
    label: 'Daily',
    items: [
  ...
  return (
    <>
      <aside className="hidden lg:flex lg:flex-col w-64 shrink-0 bg-white border-r border-slate-200 h-screen sticky top-0">
        ...
        <nav className="flex-1 overflow-y-auto scroll-thin py-3 px-3 space-y-4" aria-label="Admin sections">
          {NAV_GROUPS.map((group, gi) => (
            <div key={group.label || gi} className="space-y-0.5">
              {group.label && <p className="px-3 pb-1 text-[10px] font-bold uppercase tracking-widest text-slate-400">{group.label}</p>}
